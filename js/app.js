// ═══════════════════════════════════════════════
//  Global Money v4.2 — js/app.js
//  Application entry point — wires all modules
// ═══════════════════════════════════════════════
...
// ── Page Navigation ───────────────────────────────
function navigateTo(page, fromBack = false) {
  ...
  // Push to internal history only when not navigating back
  if (!fromBack) {
    if (_pageHistory[_pageHistory.length - 1] !== page) {
      _pageHistory.push(page);
      if (_pageHistory.length > 30) _pageHistory.shift();
    }
  }

  if (page === "rates")       renderMultiTable();
  ...
  if (page === "crypto")      { state.setCryptoPageOpen(true); renderCryptoPrices(); _startCryptoPageRefresh(); }
  else if (state.currentPage !== "crypto") state.setCryptoPageOpen(false);
  if (page === "blackmarket") {
    import("./blackmarket.js").then(({ syncBlackMarketRates, fetchAllParallelRates, renderBlackMarket }) => {
      syncBlackMarketRates();
      fetchAllParallelRates(); // جلب أسعار حقيقية من API
      renderBlackMarket();
    });
  }
...
// iOS: show install guide if not already installed
...
function updateMarketStatus() {
  // Forex: open Mon 00:00 UTC to Fri 22:00 UTC approx
  const now = new Date();
  ...
  // Metals: roughly same as Forex (COMEX hours)
...
// ── Top Movers — Real 24h data from Binance ──────
// (Optimized version defined below after _startCryptoPageRefresh)
...
// ── Performance: debounce top movers rendering ────
...
// Init market status on load and periodically
document.addEventListener("DOMContentLoaded", () => {
  // Run storage health check first to recover any corrupt data
  initStorageHealth();

  // Initialize IndexedDB layer + migrate from LocalStorage (non-blocking)
  initDatabase().catch(() => {});

  // Init analytics consent (GDPR) — loads GA only if user agrees
  initConsent();
