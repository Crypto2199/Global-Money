...
//  Real live data: Fiat + Crypto (Binance WS) + Metals
//  Added: Retry Logic, Timeout Handling, Fallback Handling,
//         Request Validation, Analytics hooks
//  Behavior-compatible with previous version.
// ═══════════════════════════════════════════════
...
async function fetchWithTimeout(url, options = {}, timeoutMs = FETCH_TIMEOUT_MS) {
  ...
  if (!_isValidUrl(url)) {
    logApiError("fetchWithTimeout", new Error("Invalid URL"), { url });
  ...
  try {
    ...
    logApiError("fetchWithTimeout", err, { url, isTimeout });
...
// ── Analytics Hook ────────────────────────────────
// Called after fresh rates are loaded; non-blocking
let _analyticsModule = null;
async function _notifyAnalytics() {
  try {
    if (!_analyticsModule) {
      _analyticsModule = await import("./analytics.js");
    }
    _analyticsModule.onRatesLoaded(
      state.fiatRates,
      state.cryptoRates,
      { ...getMetalsRates() }
    );
...
export async function loadFiat() {
  ...
  if (stale?.rates) {
    ...
    if (updEl) {
      ...
      updEl.textContent = tr("offlineNote");
    }
...
const _binanceSymbols = ["btcusdt","ethusdt","bnbusdt","solusdt","xrpusdt","adausdt","dogeusdt","dotusdt","maticusdt"];
const _symMap = {
  btcusdt:"BTC", ethusdt:"ETH", bnbusdt:"BNB", solusdt:"SOL",
  xrpusdt:"XRP", adausdt:"ADA", dogeusdt:"DOGE", dotusdt:"DOT", maticusdt:"MATIC"
};
  ...
  try {
    ...
    _binanceWs.onmessage = (evt) => {
      try {
        ...
        if (isNaN(price) || price <= 0) return;  // extra validation

        const rates = { ...state.cryptoRates };
        rates[sym]  = price;
        rates.USDT  = rates.USDT || 1; // USDT is the quote currency for all pairs (~1 USD)
    ...
    _binanceWs.onclose = () => {
      ...
      // Exponential back-off for reconnect: 5s, 10s, 20s …
...
async function _fallbackCryptoHTTP() {
  ...
  if (data && typeof data === "object") {
    ...
    if (Object.keys(rates).length > 0) {
      rates.USDT = rates.USDT || 1; // ensure USDT base for conversions
      state.setCryptoRates(rates);
  ...
  const symbolMap    = { BTCUSDT:"BTC", ETHUSDT:"ETH", BNBUSDT:"BNB", SOLUSDT:"SOL", XRPUSDT:"XRP" };
  try {
...
export function startAutoRefresh(onRefresh) {
  ...
  setInterval(async () => {
    try {
      ...
      logApiError("autoRefresh:fiat", err);
    }
  }, 5 * 60 * 1000);

  // Crypto WS auto-reconnects; poll as backup every 60s
  setInterval(async () => {
    if (!_wsConnected) await _fallbackCryptoHTTP();
  }, 60_000);

  // Metals: every 5 minutes
  setInterval(async () => {
