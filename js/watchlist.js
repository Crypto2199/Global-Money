...
function _buildWatchlistControls(lang) {
  ...
  addFiatBtn.addEventListener("click", () => {
    const sym = fiatInput.value.trim().toUpperCase();
    if (!sym) return;
    const res = addToWatchlist(sym, "fiat");
    if (res.ok) {
      fiatInput.value = "";
  ...
  addCryptoBtn.addEventListener("click", () => {
    ...
    const res = addToWatchlist(sym, "crypto");
    if (res.ok) {
      showToast(lang === "ar" ? `✅ تمت إضافة ${sym}` : `✅ Added ${sym}`);
      renderWatchlist();
    } else if (res.reason === "duplicate") {
...
function _buildWatchItem(item, idx, total, lang) {
  ...
  if (idx > 0) {
    ...
    upBtn.addEventListener("click", () => { reorderWatchlist(idx, idx - 1); renderWatchlist(); });
    moveWrap.appendChild(upBtn);
  }
  if (idx < total - 1) {
    const dnBtn = el("button", "↓", { className: "wl-move-btn" });
    dnBtn.addEventListener("click", () => { reorderWatchlist(idx, idx + 1); renderWatchlist(); });
